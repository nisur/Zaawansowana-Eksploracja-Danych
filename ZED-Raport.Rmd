...
```{r, echo=TRUE, PowtarzalnoscWynikow}
set.seed(23)
```

```{r, echo=TRUE, cache=TRUE, LadowanieDanych}
...
```{r, echo=TRUE, cache=TRUE, CzyszczenieWartosciPustych}
dane <- dane %>% 
  replace(is.na(.), 0)
```

```{r, echo=TRUE, cache=TRUE, CzyszczenieDanych}
dane <- dane %>% 
  filter(!is.na(res_name), 
         !res_name %in% c('DA','DC','DT','DU','DG','DI','UNK','UNX','UNL','PR','PD','Y1','EU','N','15P','UQ','PX4','NAN')) 
```

```{r, echo=TRUE, cache=TRUE, WartosciUnikatowe}
dane <- dane %>% 
  distinct(pdb_code, res_name)
```


```{r, echo=TRUE, cache=TRUE, PodsumowanieOgolne}
summary(dane)
```
...
```{r, echo=TRUE, cache=TRUE, CzyszczenieDanych2}
dane <- dane %>%
...
```{r, echo=TRUE, cache=TRUE, CzyszczenieDanych3}
dane <- dane %>%
...
```{r, echo=TRUE, PodsumowanieKlasPart1}
podsumowanie <- dane %>%
  group_by(res_name) %>%
...
```{r, echo=TRUE, PodsumowanieKlasPart3}
...
```{r, echo=TRUE, cache=TRUE, Korelacja}
